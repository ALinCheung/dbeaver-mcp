 ...
 * 根据连接参数生成缓存 key
 */
function getRedisCacheKey(info: RedisConnectionInfo): string {
  return `${info.host}:${info.port}:${info.database}:${info.password}`;
}

/**
 * 获取或创建 Redis 客户端（带缓存）
 */
function getRedisClient(info: RedisConnectionInfo): Redis {
  const key = getRedisCacheKey(info);

  // 如果缓存存在且已连接，直接返回
  if (redisClientCache.has(key)) {
    const client = redisClientCache.get(key)!;
    if (client.status === "ready" || client.status === "connect") {
      return client;
    }
  }

  // 如果缓存已满，清理最旧的连接（简单 LRU）
  if (redisClientCache.size >= MAX_CACHE_SIZE) {
    const firstKey = redisClientCache.keys().next().value;
    if (firstKey) {
      const oldClient = redisClientCache.get(firstKey);
      if (oldClient) {
        oldClient.disconnect(false);
      }
      redisClientCache.delete(firstKey);
    }
  }
  ...
  redisClientCache.set(key, redis);
  return redis;
}
...
export function clearRedisCache(): void {
  for (const client of redisClientCache.values()) {
...
/**
 * 异步连接 Redis 客户端（确保连接就绪）
 */
async function ensureRedisConnection(info: RedisConnectionInfo): Promise<Redis> {
  const client = getRedisClient(info);
...
/**
 * 连接 Redis（兼容旧接口，内部使用缓存）
 * @deprecated 使用 getRedisClient 或 ensureRedisConnection 替代
 */
export async function redisConnect(info: RedisConnectionInfo): Promise<Redis> {
  return await ensureRedisConnection(info);
}
...
export async function runRedisQuery(
  ...
  const redis = await ensureRedisConnection(info);
